...
if __name__ == '__main__':
    ...
    bitext_f_e = [[sentence.strip().split() for sentence in pair] for pair in
              islice(zip(open(f_data), open(e_data)), opts.num_sents)]
    bitext_e_f = [[sentence.strip().split() for sentence in pair] for pair in
              islice(zip(open(e_data), open(f_data)), opts.num_sents)]

    # Train model and get probability distribution
    probabilities_f_e = train_alignment(bitext_f_e, 10)
    probabilities_e_f = train_alignment(bitext_e_f, 10)
