...
for r, d, f in os.walk('../data/scisumm-corpus'):
    for f_ in f:
        if '.xml' in f_ and f_ not in file_names:
            file_names.append(f_)
...
for art_xml in art_xmls:
    try:
        ...
        xml_string = open(art_xml, 'rb').read()
        soup = BeautifulSoup(xml_string, 'html.parser')

        # Get article content
        article_xmls = soup.find_all('section')
        article_str = ''

        for sect in article_xmls:
            sect = sect.find_all('s')
            sect_string = ''.join([s.string.replace('\n', '').strip() for s in sect])
            article_str += sect_string
        ...
        try:
            ...
            abs_sents = abstract.find_all('s')
        except:
            continue

        abs_string = ''.join([s.string.replace('\n', '').strip() for s in abs_sents])
        summaries.append(abs_string)
        articles.append(article_str)
